...
public final class CoreMLLayoutClassifier {
    private let logger = Logger(subsystem: "com.chernistry.rightlayout", category: "CoreML")
    ...
    /// Whether the CoreML model was successfully loaded.
    /// When `false`, `predict()` always returns `nil` and the system
    /// falls back to heuristic-only detection (N-gram + Ensemble).
    public private(set) var modelAvailable: Bool = false
    ...
    public init() {
        ...
        do {
            ...
            if let compiledURL = CoreMLLayoutClassifier.compiledModelURL {
                ...
                logger.warning("CoreML model not found — deep path disabled, falling back to heuristics")
            }
        } catch {
            logger.error("Failed to load CoreML model: \(error.localizedDescription) — deep path disabled")
        }
    }
